...
class ML(BasicDataset):
    ...
    def getSparseGraph(self,use_PA):
        ...
        if self.Graph is None:
            ...
            print('use_PA',use_PA)
            if use_PA:
                Graph_tmp = pd.read_csv("regional_central_node_douban.csv")
                ...
                Graph_tensor[:3000,3000:] = torch.zeros(3000, 3000)
                Graph_tensor[3000:,:3000] = torch.zeros(3000,3000)
            ...
            User_attri = np.array(u_featuresData[:][0])
            trainUniqueUsers = np.unique(User_attri)
            attri_User = np.array(u_featuresData[:][1])

            item_attri  = np.array(v_featuresData[:][0])
            Userattri_unique = np.unique(item_attri)
            Item_attri  = np.array(v_featuresData[:][1])
            ...
            UserFeaturesNet = csr_matrix((np.ones(len(User_attri)), (User_attri, attri_User)),
                                                  shape=(n_users, m_items))
            R_u = UserFeaturesNet.tolil()
            
            ItemsFeaturesNet = csr_matrix((np.ones(len(item_attri)), (item_attri, Item_attri)),
                                                  shape=(n_users, m_items))
