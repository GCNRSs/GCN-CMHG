# -*- coding: utf-8 -*-
"""
Created on Sat Aug 14 08:00:19 2021
...
def load_data_monti(dataset, testing=False):
    ...
    if dataset == 'flixster':
        ...
        u_features = Wrow#+np.eye(num_users)
        v_features = Wcol#+np.eye(num_items)
...
def load_official_trainvaltest_split(dataset, testing=False):
    ...
    if dataset =='ml_100k':
        ...
        for movie_id, g_vec in zip(movie_df['movie id'].values.tolist(), movie_df[genre_headers].values.tolist()):
            ...
            if movie_id in list(v_dict.keys()):
...
if DATASET == 'flixster' or DATASET == 'douban' or DATASET == 'yahoo_music':
    u_features, v_features, adj_trainR, train_labels, train_u_indices, train_v_indices, \
...
adj_train_int = sp.csr_matrix(adj_trainR, dtype=np.int32)
print('adj_train_int',adj_train_int)
adj_trainR = sp.csr_matrix(adj_train_int == 5, dtype=np.float32)
#print(support_unnormalized)
print('----------------------')
print(adj_trainR.shape)
print(type(adj_trainR))
print(adj_trainR)
#u_features = list(np.array(u_features))
from torch.utils.data import TensorDataset
x,y = torch.tensor(np.nonzero(u_features), dtype=torch.int)
l = TensorDataset(x,y)
u_features = torch.tensor(l).tolist()
print('csr_matrix:',u_features,type(u_features))
#print('v_features:',v_features)
print('csr_matrix:',len(u_features),len(u_features[:][0]))


list1 = []
for i in range(len(u_features)):
    list1.append(u_features[i][0])
...
for i in range(3000):
    if i not in list1:
        list2.append([i,i])
u_features = u_features+list2
u_features_df = pd.DataFrame(u_features)
print('list2',list2)
u_features_df = u_features_df.sort_values(by=[0] , ascending=True) 
#print('u_features_df',u_features_df)

x,y = torch.tensor(np.nonzero(v_features), dtype=torch.int)
l = TensorDataset(x,y)
v_features = torch.tensor(l).tolist()
v_features_df = pd.DataFrame(v_features)

list1 = []
for i in range(len(v_features)):
    list1.append(v_features[i][0])
...
for i in range(3000):
    if i not in list1:
        list2.append([i,i])
v_features = v_features+list2
v_features_df = pd.DataFrame(v_features)
v_features_df = v_features_df.sort_values(by=[0] , ascending=True) 
...
u_features_df.to_csv(file_dir + '/u_features20232.txt', sep='\t',index=False, header = False)
v_features_df.to_csv(file_dir + '/v_features20232.txt', sep='\t',index=False, header = False)
#np.savetxt('u_features',u_features)
